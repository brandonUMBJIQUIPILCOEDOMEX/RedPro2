...
$(".like_btn").click(function () {
    ...
    var post_reaction = $(this).data('postReaction');




    var fiveBtn = $(button).siblings(".five_btn");
    ...
    $.ajax({
        ...
        method: 'post',
        dataType: 'json',
        data: { post_id: post_id_v, post_reaction: post_reaction },
        ...
        success: function (response) {
            ...
            if (response.status) {
                ...
                var five_btn = $(button).siblings('.five_btn');
                //var five_btn = document.querySelector(".five_btn");
                var computedStyle = window.getComputedStyle(five_btn[0]);

                /*
                var likeBtn = document.querySelector(".unlike_btn");
                var computedStyle = window.getComputedStyle(likeBtn); */

               //**** alert("/" + computedStyle.display + "/");

                if (computedStyle.display == 'none') {
                    //alert("tiene reaccion five.");




                   // var icon2 = document.querySelector(".five_btn");
                    //icon2.style.display = ""; // o "inline-block"


                    var icon2 = five_btn[0];
                    icon2.style.display = ""; // o "inline-block"


                    /*
            
                    si le pongo none se quita el icono
            
                    var icon2 = document.querySelector(".five_btn");
                    icon2.style.display = "none"; // o "inline-block"
                    */


                    //var icon = document.querySelector(".unfive_btn");
                    //icon.style.display = "none"; // o "inline-block"
                    // quitar la otra reaccion


                    var unfiveBtn = $(button).siblings(".unfive_btn");

                    var icon = unfiveBtn[0];
                    ...
                    // afuerza debo crear otro evento por que si no se quitan reaaciones de otras publicaiciones
                   $(button).siblings(".unfive_btn").trigger("click");
...
$(".five_btn").click(function () {
    ...
    var likeBtn = $(button).siblings('.like_btn');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status) {
                ...
                $(button).attr('disabled', false);
                $(button).hide()
                $(button).siblings('.unfive_btn').show();

                $('#likecount' + post_id_v).text($('#likecount' + post_id_v).text() - (-1));
                // location.reload();


                /* ======== VERIFICACION DE REACCION MULTIPLE ======== */


                //var like_btn = document.querySelector(".like_btn");
                //var computedStyle = window.getComputedStyle(like_btn);


                var likeBtn = $(button).siblings('.like_btn');
                var computedStyle = window.getComputedStyle(likeBtn[0]);
               ...
               //****  alert("/" + computedStyle.display + "/");
                ...
                if (computedStyle.display == 'none') {
                    //**** alert("tiene reaccion like.");




                    // var icon2 = document.querySelector(".like_btn");
                    // icon2.style.display = ""; // o "inline-block"
                    ...
                    */
                    ...
                    //var icon = document.querySelector(".unlike_btn");
                    //icon.style.display = "none"; // o "inline-block"




                    var unlikeBtn = $(button).siblings('.unlike_btn');
                    var icon = unlikeBtn[0];
                    icon.style.display = "none";



                    // quitar la otra reaccion
                    //$(".unlike_btn").trigger("click");

                    /* ======== ======================== ======== */

                    $(button).siblings(".unlike_btn").trigger("click");
