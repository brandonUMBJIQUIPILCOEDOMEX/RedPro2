...
/* ============= ELIMINAR COMENTARIO ============= */
if (isset($_GET['deletecomment']) && isset($_GET['comment'])) {
    $post_id = $_GET['deletecomment'];
    $comment = $_GET['comment'];
    if (deleteComment($post_id, $comment)) {
        header("location:{$_SERVER['HTTP_REFERER']}");
    } else {
        echo "something went wrong";
    }
...
if (isset($_GET['addpostshared'])) {


    if (createPost($_POST, '', $_POST['modal_id_post'])) {
        header("location:../../?new_post_added");
    } else {
        echo "something went wrong";
        $_SESSION['error'] = $response;
