...
if(isset($_SESSION['Auth']) && $user['ac_status']==1 && !$pagecount){
    ...
    showPage('navbar');
    showPage('wall');}elseif(isset($_SESSION['Auth']) && $user['ac_status']==0 && !$pagecount){
...
}elseif(isset($_SESSION['Auth']) && isset($_GET['lechistory']) && $user['ac_status']==1){
    showPage('header',['page_title'=>'Historial de lecturas']);
    showPage('navbar');
    showPage('lec_history');  
}

elseif(isset($_SESSION['Auth']) && isset($_GET['lecrealtime']) && $user['ac_status']==1){
    showPage('header',['page_title'=>'Tiempo real']);
    showPage('navbar');
    showPage('lec_realtime');  
}
...
elseif(isset($_SESSION['Auth']) && isset($_GET['u']) && $user['ac_status']==1){
    ...
    showPage('header',['page_title'=>'Strawberry - SignUp']);
    showPage('signup');
}elseif(isset($_GET['login'])){
   
    showPage('header',['page_title'=>'Strawberry - Login']);
    showPage('login');
}elseif(isset($_GET['forgotpassword'])){
    
    showPage('header',['page_title'=>'Strawberry - Forgot Password']);
